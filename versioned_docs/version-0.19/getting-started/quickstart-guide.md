...
```
# Standard mode
curl -sL "https://github.com/kserve/kserve/releases/download/v0.19.0/kserve-standard-mode-full-install-with-manifests.sh" | bash

# Knative mode (serverless)
curl -sL "https://github.com/kserve/kserve/releases/download/v0.19.0/kserve-knative-mode-full-install-with-manifests.sh" | bash
```
</details>
...
```bash
# Standard mode
./hack/kserve-install.sh --kserve-version v0.19.0 --type kserve --standard

# Knative mode (serverless)
./hack/kserve-install.sh --kserve-version v0.19.0 --type kserve --knative
```
...
```bash
# Standard mode
./hack/kserve-install.sh --kserve-version v0.19.0 --type kserve,localmodel --standard

# Knative mode (serverless)
./hack/kserve-install.sh --kserve-version v0.19.0 --type kserve,localmodel --knative
```
...
```
curl -sL "https://github.com/kserve/kserve/releases/download/v0.19.0/llmisvc-full-install-with-manifests.sh" | bash
```
</details>
...
```bash
./hack/kserve-install.sh --kserve-version v0.19.0 --type llmisvc
```
...
curl -fsSL https://github.com/kserve/kserve/releases/download/v0.19.0/kserve-standard-mode-dependency-install.sh | bash

# OR

# Option B: Knative Mode Dependencies
curl -fsSL https://github.com/kserve/kserve/releases/download/v0.19.0/kserve-knative-mode-dependency-install.sh | bash
```
...
curl -fsSL https://github.com/kserve/kserve/releases/download/v0.19.0/llmisvc-dependency-install.sh | bash


# Install CRDs
kubectl apply -f https://github.com/kserve/kserve/releases/download/v0.19.0/kserve-crds.yaml
# Install Controllers
...
kubectl apply -f https://github.com/kserve/kserve/releases/download/v0.19.0/kserve-cluster-resources.yaml
```
...
./hack/kserve-install.sh --kserve-version v0.19.0 --type kserve,llmisvc,localmodel --standard

# All components (Knative mode)
./hack/kserve-install.sh --kserve-version v0.19.0 --type kserve,llmisvc,localmodel --knative
```
...
- 📖 **[Deploy Your First LLM with LLMInferenceService](genai-first-llmisvc)** - Serve an LLM with advanced features (prefix-aware routing, disaggregated serving)
- 📖 **[Deploy Your First Predictive Model with InferenceService](predictive-first-isvc)** - Serve a traditional ML model using InferenceService
