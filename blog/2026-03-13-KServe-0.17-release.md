...
LLMInferenceService integrates with [**Gateway Inference Extension (GIE) v1.2.0**](https://gateway-api.sigs.k8s.io/geps/gep-3567/), a Kubernetes SIG project that extends the Gateway API with AI-specific routing capabilities. At the heart of this integration is the **Endpoint Picker Pod (EPP)**, an intelligent scheduler that routes requests based on real-time KV-cache state.

The EPP uses a scoring mechanism that tracks KV cache blocks via ZMQ events and applies weighted scoring:
- **prefix-cache-scorer** (weight: 2.0): Routes requests to pods with matching cached prefixes, reducing Time To First Token (TTFT)
- **load-aware-scorer** (weight: 1.0): Distributes load evenly across available pods

This means repeated or similar prompts are automatically routed to the same backend pod that already has the relevant KV cache loaded, significantly improving latency for multi-turn conversations and RAG workloads.
...
A new **autoscaling API** has been added to LLMInferenceService with support for the **Workload Vertical Autoscaler (WVA)** alongside [KEDA](https://keda.sh)-based horizontal scaling. This enables intelligent scaling decisions based on LLM-specific metrics such as `vllm:num_requests_running`, with support for both **Prometheus-based** polling and **OpenTelemetry-based** push metrics.
