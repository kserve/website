...
* Allow client send and receive tensor data in binary format for HTTP/REST request, see [binary tensor data extension docs](https://kserve.github.io/archive/0.14/modelserving/data_plane/binary_tensor_data_extension/).
...
You can follow [local model cache tutorial](https://kserve.github.io/archive/0.14/modelserving/storage/modelcache/localmodel/) to cache LLMs on local NVMe of your GPU nodes and deploy LLMs with `InferenceService` by loading models from local cache to accelerate the container startup. 
...
Read the [documentation](https://kserve.github.io/archive/0.14/modelserving/storage/huggingface/hf/) for more details.
