...
## Server-Side TLS for Transformers

KServe transformers can serve HTTPS natively using TLS certificates. When SSL certificate and key files are provided, the server automatically switches from HTTP (port 8080) to HTTPS (port 8443).

### Configuration

TLS is configured via command-line arguments or environment variables:

| Argument | Environment Variable | Description |
|----------|---------------------|-------------|
| `--ssl_certfile` | `KSERVE_TLS_CERT_FILE` | Path to the SSL certificate file |
| `--ssl_keyfile` | `KSERVE_TLS_KEY_FILE` | Path to the SSL private key file |
...
|-----------|--------------|-------------|
| Not set | Not set | 8080 (HTTP) |
| Set | Not set | 8443 (HTTPS) |
| Set | Overridden | Custom port (HTTPS) |

### Readiness Probes

When using server-side TLS, the container must advertise the HTTPS port via `containerPort` in the pod spec. The default readiness probe uses `containerPort` for TCP health checks. If no port is defined, it falls back to 8080, which will fail if the server is listening on 8443.

For transformers with TLS enabled, set the port in the InferenceService spec:
