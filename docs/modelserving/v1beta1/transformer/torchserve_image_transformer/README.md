...
and gets back a response which then passes to `postproces` handler. KServe automatically fills in the `predictor_host` for `Transformer` and handle the call to the `Predictor`, for gRPC you can pass the `protocol` argument with `grpc-v2`.
...
## Deploy the InferenceService calling Predictor with REST protocol
...
Please create the `InferenceService` with following yaml which includes a Transformer and a Triton Predictor, the Transformer calls out to
