...
KServe now integrates with [**KEDA**](https://keda.sh) (Kubernetes Event-Driven Autoscaling) which is an optional component, enabling advanced scaling strategies:

- Scale based on LLM-specific metrics such as KV cache utilization or token throughput.
