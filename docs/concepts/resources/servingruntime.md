...
description: Learn how to use ServingRuntimes in KServe to define reusable model serving environments for various model formats.
---
...
| kserve-huggingfaceserver  | Hugging Face Transformer Models     |
...
:::note
`ServingRuntimes` support the use of template variables of the form `\{\{.Variable\}\}` inside the container spec. These should map to fields inside an
...
...
```
Then, the version of the `supportedModelFormat` must also match. In this example, `kserve-sklearnserver` would not be eligible for selection since
...
<Tabs>
<TabItem value="previous-schema" label="Previous Schema" default>
...
<TabItem value="equivalent-new-schema" label="Equivalent New Schema">
