...
KServe `InferenceService` creates [Knative Service](https://knative.dev/docs/serving/spec/knative-api-specification-1.0/#service) under the hood to instantiate a 
...
that should be exposed and the type of protocol to use. If you are using `Standalone KServe`, it uses the `Gateway` in `knative-serving` namespace,
if you are using `Kubeflow KServe`(KServe installed with Kubeflow), it uses the `Gateway` in `kubeflow` namespace e.g on GCP the gateway is protected behind `IAP` with [Istio 
...
2. KServe creates a `Istio virtual service` to specify routing rule for predictor, transformer, explainer and canary
```bash
...
spec:
  ...
  - match:
    ...
    - destination:
      ...
      weight: 100
```
- KServe creates the routing rule based on uri prefix according to [KServe V1 data plane](./README.md#data-plane-v1) and traffic
is forwarded to `KServe Predictor` if you only have `Predictor` specified on `InferenceService`,
note that if you have custom container and the endpoint is not conforming to the protocol you get `HTTP 404` when you hit the KServe
...
```
5. The `queue proxy` sends single or multi-threaded requests that the `kserve container` can handle at a time.
...
6. Finally The `queue proxy` sends traffic to the `kserve-container`.
...
![sklearn-iris distributed tracing](../images/sklearn-iris-tracing.png)
