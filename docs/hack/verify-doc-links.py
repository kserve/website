...
GITHUB_REPO = env.get("GITHUB_REPO", "https://github.com/kserve/website/")
BRANCH = "main"

github_io_base_url = "https://kserve.github.io/website"
...
github_repo_main_path = "{}/blob/{}".format(GITHUB_REPO.rstrip("/"), BRANCH)
...
def get_links_from_md_file(md_file_path: str) -> [(int, str, str)]:  # -> [(line, link_text, URL)]
    ...
    md_file_content = re.sub(
        r"\[([^]]+)\]\((?!http|#|/)([^)]+)\)",
        r"[\1]({}/{}/\2)".format(github_repo_main_path, folder).replace("/./", "/"),
        md_file_content)

    # replace links that are relative to the project root, i.e. [link text](/sdk/FEATURES.md)
    md_file_content = re.sub(
        r"\[([^]]+)\]\(/([^)]+)\)",
        r"[\1]({}/\2)".format(github_repo_main_path),
        md_file_content)
    ...
    if file_line_text_url_404:
        ...
        for (file, line, text, url, status) in sorted(file_line_text_url_404):
            print("{}:{}: {} -> {}".format(
                ...
                url.replace(github_repo_main_path, ""), status))
